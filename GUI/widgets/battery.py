...
class Battery(QWidget):
    ...
    def initUI(self, battNum):
        ...
        if (battNum == 1):
            ...
            avgTemp = (BATTERY_1 + BATTERY_2 +
                       BATTERY_3 + BATTERY_4 + BATTERY_5)/5
            temperature.setText("Average Temperature: " + str(avgTemp) + " °C")
            avgVolt = (VOLTAGE_BATT_1 + VOLTAGE_BATT_2 +
                       VOLTAGE_BATT_3 + VOLTAGE_BATT_4 + VOLTAGE_BATT_5)/5
            voltage.setText("Average Voltage: " + str(avgVolt))
            avgCurrent = (CURRENT_BATT_1 + CURRENT_BATT_2 +
                          CURRENT_BATT_3 + CURRENT_BATT_4 + CURRENT_BATT_5)/5
