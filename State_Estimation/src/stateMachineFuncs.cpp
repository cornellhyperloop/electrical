...
states decelerate(double traveledDist, double sensorVelocity)
{
  ...
  // TODO: Implement and return correct state
  closeBrakes();
  if (abs(sensorVelocity) <= sensorStopEps) // TODO: FIX FOR WHEN DECELERATING AND STOPPING TOO EARLY
  {
    return Stop;
  }
  else if (abs(sensorVelocity) > sensorStopEps && checkDistance(totalDist, traveledDist, stopRange))
  {
    return Emergency;
  }
  // else
...
states stop(double traveledDist)
{
  ...
  // Add manual interrupt to go into Emergency state
  // TODO: Implement and return correct state
  if (checkDistance(totalDist, traveledDist))
  {
    return PodOff;
  }
  else
